...
class APIController extends Controller
{
    ...
    public function getUsers(    Request $request, $id = null) {
        if ($request->isMethod ('get')) { // Check if the incoming HTTP Request Method/Verb is 'GET'
            ...
            } elseif ($header != 'Bearer         eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJzdWIiOiIxMjM0NTY3ODkwIiwibmFtZSI6IkFtaXQgR3VwdGEiLCJpYXQiOjE1MTYyMzkwMjJ9.cNrgi6Sso9wvs4GlJmFnA4IqJY4o2QEcKXgshJTjfNg') { // JWT token generated by (we wrote in "Mohamed Ashraf" to generate this token in the website): https://jwt.io/    // User must send an "Authorization" HTTP Header with all their HTTP Requests with this value (Bearer Token (JWT)): "Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJzdWIiOiIxMjM0NTY3ODkwIiwibmFtZSI6IkFtaXQgR3VwdGEiLCJpYXQiOjE1MTYyMzkwMjJ9.cNrgi6Sso9wvs4GlJmFnA4IqJY4o2QEcKXgshJTjfNg"
    ...
    public function addUser(Request $request) {
        if ($request->isMethod('post')) { // Check if the incoming HTTP Request Method/Verb is 'POST'
            ...
            // Add/Save the submitted new user data in `users` table ('name', 'email', '       pa ssword' fields must be submitted in Postman!)
            $user = new User;
            ...
            // Send back the newly submitted user data of the successful operation to the user again! (Note: This a well-known convention of APIs to send back the successful accepted data!)
            return response()->json([ // Note: $user is AUTOMATICALLY converted from Laravel/PHP object to JSON! But w    e'll manually convert it to JSON using    return response()->json()    to show the "users" JSON property key/name in the response! Try the TWO cases and check the difference in Postman in "Pretty" tab!    // JSON Responses: https://laravel.com/docs/9.x/responses#json-responses
                'message' => 'User added successfully!',
                'user'    => $user
            ], 201); // 201 HTTP Status Code: Created    // With POST requests, 201 Created HTTP Status Code is sent with the HTTP Response

        } else { // if the user is using the wrong dedicated HTTP Request Method/Verb for their request (i.e. User has us        ed an HTTP Request Method/Verb other than 'POST')
            $message = 'You\'re using an incorrect/invalid HTTP Request Method/Verb to access this route/endpoint in our API!';
    ...
    public function addMultipleUsers(Request $request) {
        if ($request->isMethod('post')) { // Check if the incoming HTTP Reque     st Method/Verb is 'POST'
            $userData = $request->input();

            // Validation (Laravel Validation)    // Manually Creating Validators: https://laravel.com/docs/9.x/validation#manually-creati            ng-validators
            ...
            // Use a foreach loop because there are multiple JSON data/objects submitted in one go in the request    // Note: In Postman, JSON data ar       e  submitted using Curly Braces {} and a "users" Wrapping Object and Square Brackets [])
    ...
    public function updateUserDetails(Request $request, $id = null) { // Note: Query String Paramets (like 'id') can be submitted in THREE ways: using "Body" tab in Postman (as JSON data), as a URL      Query String Parameters (e.g. www.example.com?id=3) and using Postman "Params" tab
        if ($request->isMethod('put')) { // Check if the incoming HTTP Request Method/Verb is 'PUT'
            ...
            // Validation (Laravel Validation)    // Ma        nually Creating Validators: https://laravel.com/docs/9.x/validation#manually-creating-validators
            $rules = [
                // Fields/Column Names
                'name'     => 'required|regex:/^[\pL\s\-]+$/u', // regex:pattern: https://laravel.com/docs/9.x/validation#rule-regex
            ...
            User::where('id', isset($id) ? $id : $userData['id'])->update([ // if the id is sent as a URL Query String Parameter, take its value a       s  is, or else, take its value from the JSON "Body"
                'name'     => $userData['name'],
            ...
            $message = 'You\'re using an incorrect/invalid HTTP Reques        t Method/Verb to access this route/endpoint in our API!';
    ...
    // Update the 'name' ONLY of a Single user (just a portion of a resource) (PATCH)    // API Endpoint:    PATCH http://127.0.0.1:8000/api/update-user-name Or PATCH http://127.0.0.1:8000/api/update-user-name/21    // Note: In Post        man, JSON data are submitted using Curly Braces {} and a "users" Wrapping Object and Square Brackets [])    // 'id' and 'name' fields must be submitted by the user (from Postman). Note: Data can be posted/sent/submitted in Postman using a GET/POST/PUT/PATCH/DELETE request using TWO ways: First: "form-data" (resembles the HTML Form <input> fields (key/value pairs) and their "name" and "value" <input> HTML tag attributes, and you can retrieve those data in the backend using the Superglobals $_POST and $_FILES), Second: "raw" (through which you can decide the data type of the whole HTTP Request Body (e.g. JSON, HTML, ...etc), and you can retrieve those data in the backend using, for example if data were JSON, json_decode() function and then access the JSON key/value pairs as follows:    $jsonImageAttributes->filename    )
    public function updateUserName(Request $request, $id = null) { // Note: Query String Paramets (like 'id') can be submitted in THREE ways: using "Body" tab in Postman (as JSON data), as a URL Query String Parameters (e.g. www.example.com?id=3) and us     ing Postman "Params" tab
        if ($request->isMethod('patch')) { // Check if the incoming HTTP Request Method/Verb is 'PATCH'
            $userData = $request->input();

            // Validation (Laravel Validation)     // Manually Creating Validators: https://laravel.com/do        cs/9.x/validation#manually-creating-validators
            $rules = [
                // Fields/Column Names
                'name'     => 'required|regex:/^[\pL\s\-]+$/u', // regex:pattern: https://laravel.com/docs/9.x/validation#rule-regex
            ];
            ...
            User::where('id', isset($id) ? $id : $userData['id'])->update(['name' => $userData['name']]); // if the id is sent as a URL Query String Parameter, ta       ke  its value as is, or else, take its value from the JSON "Body"
            ...
            $message = 'You\'re using an incorrect/invalid HTTP Request Method/Verb to access this route/endpoi        nt in our API!';
    ...
    // Delete Multiple users (DELETE)    // API Endpoint:    DELETE http://127.0.0.1:8000/api/delete-multiple-users Or DELETE http://127.0.0.1:8000/api/delete-multiple-users/11,13,20    // Note: In Postman, JSON data are submitted using Curly Braces {} and an "ids" Wrapping Object and Square Bracke        ts [])    // 'ids' field must be submitted by the user (from Postman). Note: Data can be posted/sent/submitted in Postman using a GET/POST/PUT/PATCH/DELETE request using TWO ways: First: "form-data" (resembles the HTML Form <input> fields (key/value pairs) and their "name" and "value" <input> HTML tag attributes, and you can retrieve those data in the backend using the Superglobals $_POST and $_FILES), Second: "raw" (through which you can decide the data type of the whole HTTP Request Body (e.g. JSON, HTML, ...etc), and you can retrieve those data in the backend using, for example if data were JSON, json_decode() function and then access the JSON key/value pairs as follows:    $jsonImageAttributes->filename    )
    public function deleteMultipleUsers(Request $request, $ids = null) { // Note: Query String Paramets (like 'id') can be submitted in THREE ways: using "Body" tab in Postman (as JSON data), as a URL Query String Parameters (e.g. www.example.com?id=3) and using Postman "Params" tab
        if ($request->isMethod('dele     te')) { // Check if the incoming HTTP Request Method/Verb is 'DELETE'
            $userData = $request->input();

            if (isset($ids)) { // if the ids are sent as a URL Query String Parameter, take its value as is, or else, take its values from the JSON (using the "Body" tab i        n Postman)
                ...
                // Send back the newly submitted user data of the successful operation to the user again! (Note: This a well-known convention of APIs to send back the successful accepted data!)
                return response()->json([ // Note: $user is AUTOMATICALLY converted from Laravel/PHP object to JSON! But we'll manually convert it to JSON using    return response()->json()    to show the "users" JSON property key/name in the response! Try the TWO cases and check the difference in Postman in "Pretty" tab!    // JS    ON Responses: https://laravel.com/docs/9.x/responses#json-responses
                ...
                // Send back the newly submitted user data of the successful operation to the user again! (Note: This a well-known convention of APIs to send back the successful accepted data!)
                return response()->json([ // Note: $user is AUTOMATICALLY converted from Laravel/PHP object to JSON! But we'll manually convert it to JSON using    return response()->json()    to show the "users" JSON property key/name in the response! Try the TWO cases and check the difference in Postman in "Pretty" tab!    // JSON Respons    es: https://laravel.com/docs/9.x/responses#json-responses
                    'message' => 'Users deleted successfully!'
                ], 202/* 204 */); // 204 HTTP Status Code: No Content    // With DELETE requests, 204 No Content, 202 Accepted or 200 HTTP Status Code is sent with the HTTP Response    // Note: 204 No Content HTTP Status Code cancels out/hides any Messages you send back with your HTTP Response! (Your Message would disappear/vanish with your HTTP Response! Nothing at all will show up to the user!)
            }
    ...
    public function registerUser(Request $request) {
        if ($request->isMethod('post')) { // Check if the incoming HTTP Request Method/Verb is 'POST'
            ...
            // Generate a new Access Token for the user, and save it in the `access_token` column in `users` table (and overwrite/override/replace the o            ld Access Token in `access_token` column if there is any!)
            $accessToken = \Illuminate\Support\Str::random(60); // Str::random(): https://laravel.com/docs/9.x/helpers#method-str-random

            // Regist        er the new user i.e. Save the submitted user data (name, email and password) along with the newly generated API Access Token in `users` table
            $user = new User;
    ...
    public function loginUser(Request $request) {
        if ($request->isMethod('post')) { // Check if the incoming HTTP Request Method/Verb is 'POST'
            ...
            // Verify the submitted Password (compare the submitted password (N.B. It gets hashed before comparing by password_verify() function) with the "hashed" `password` in `user       s`  table). If both the submitted password and the user's password in the database table match, we generate a new Access Token for the user to use and send it with all their subsequent HTTP Requests, and update `access_token` column in `users` table with that new Access Token (we replace the old one with the new one)
            if (password_verify($userData['password'], $userDetails->password)) {
                ...
                return response()->json([ // Note: $user is AUTOMATICALLY converted from Laravel/PHP object to JSON! But we'll manually convert it to JSON using    return response()->json()    to show the "users" JSON property key/name in the response! Try the TWO cases and check the difference in Postman in "Pretty" tab!    // JSON Responses: https://laravel.com/docs/9.x/responses#json-responses
                    'status'  => true,
                    'messa    ge' => 'User logged in successfully!',
                    'token'   => $accessToken
                ], 201);  // 201 HTTP Status Code: Created    // With POST requests, 201 Created HTTP Status Code is sent with the HTTP Response
        ...
        } else { // if the user is using the wrong dedicated HTTP Request Method/Verb for thei        r request (i.e. User has used an HTTP Request Method/Verb other than 'POST')
    ...
    // Log in a user and Generate a ne            w Access Token for them Using "Passport" (POST)    // API Endpoint:    POST http://127.0.0.1:8000/api/login-user-with-passport    // Whenever a new user registers a new account using our Registration API Endpoint, we'll generate an Access Token for them to use and send with all their subsequent HTTP Requests (and will store it in the `api_token` column in `users` table). Note: Whenever the user logs in, we'll generate a new Access Token (that acts the same as a browser Session (i.e. Both Session and Access Token are sent from the client to the server with EVERY HTTP Re        quest), except the fact that there's no browser Session here!) that will replace the old one in the `api_token` column in `users` table. This token will be valid only till the user logs out and then expires (by deleting it from the `api_token` column in the `users` database table when the user logs out)    // User must send an "Authorization" HTTP Header with all their subsequent HTTP Requests with the value of the "Bearer" Accesss Token that they received in the HTTP Response when they first logged in or when they first registered their brand-new account. The "Bearer" Access Token should be sent in the following form:    "Bearer xxxxxxxxxxxxxxxxxx"    where xxxxxxxxxxxxxxxxxx is the "Bearer" Access Token value.    // Note: In Postman, JSON data are submitted using Curly Braces {} and an "ids" Wrapping Object and Square Brackets [])    // 'email' and 'password' fields must be submitted by the user (from Postman). Note: Data can be posted/sent/submitted in Postman using a GET/POST/PUT/PATCH/DELETE request using TWO ways: First: "form-data" (resembles the HTML Form <input> fields (key/value pairs) and their "name" and "value" <input> HTML tag attributes, and you can retrieve those data in the backend using the Superglobals $_POST and $_FILES), Second: "raw" (through which you can decide the data type of the whole HTTP Request Body (e.g. JSON, HTML, ...etc), and you can retrieve those data in the backend using, for example if data were JSON, json_decode() function and then access the JSON key/value pairs as follows:    $jsonImageAttributes->filename    )
        ...
        if ($request->isMethod('post')) { // Check if the incoming HTTP Request Method/Verb is 'POST'
            ...
            if (\Illuminate\Support\Facades\Auth::attempt(['email' => $userData['email'], 'password' => $userData['password']])) {
                ...
                // dd($user);
                            ...
                            // Generate a new Access Token using "Laravel Passport" Package for the newly registered user, and save it in the `access_token` column in `users` table (and overwrite/override/replace the old Access Token in `acc       es s_token` column if there is any!)
                $accessToken = $user->createToken($userData['email'])->accessToken; // Utilizing the HasApiTokens trait we're using in the User.php model    // Managing Personal Access Tokens: https://laravel.com/docs/9.x/passport#managing-personal-access-tokens
                // dd($accessToken);
                ...
                return response()->json([ // Note: $user is AUTOMATICALLY converted from Laravel/PHP object to JSON! But we'll manually convert it to JSON using    return response()->json()    to show the "users" JSON property key/name in the response! Try the TWO cases and check the difference in Postman in "Pretty" tab!    // JSON Responses: https://laravel.com/docs/9.x/responses#json-responses
                    ...
                    'token'   => $accessToken,
                    'user'    => $user
                ], 201);  // 201 HTTP Status Code: Created    // With P    OST requests, 201 Created HTTP Status Code is sent with the HTTP Response
            ...
            $message = 'You\'re using an incorrect/invalid HTTP Request Method/Verb to         a            ccess this route/endpoint in our API!';
            return response()->json([ // JSON Responses: https://laravel.com/docs/9.x/responses#json-responses
                'status'  => false,
                'message' => $message
            ], 422); // 422 HTTP Status Code: Unprocessable Content
    ...
    public function updateStock(Request $request) {
        if ($request->isMethod('post')) { // Check if the incoming HTTP Request Method/Verb is 'POST'
            ...
            if (empty($header)) { // if the user doesn't provide/send the 'Authorization' HTTP Header with their HTTP Request
                ...
                if ($header == 'Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJzdWIiOiIxMjM0NTY3ODkwIiwibmFtZSI6IkFtaXQgR3VwdGEiLCJpYXQiOjE1MTYyMzkwMjJ9.cNrgi6Sso9wvs4GlJmFnA4IqJY4o2QEcKXgshJTjfNg') { // if the user sends the correct HTTP Request Header value (i.e. correct Bearer Access Token) with their HTTP Request    // JWT token generated by (we wrote in "Mohamed Ashraf" this token in the website): https://jwt.io/    // User must send an "Authorization" HTTP Header with all their HTTP Requests with thi        s value (Bearer Token (JWT)): "Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJzdWIiOiIxMjM0NTY3ODkwIiwibmFtZSI6IkFtaXQgR3VwdGEiLCJpYXQiOjE1MTYyMzkwMjJ9.cNrgi6Sso9wvs4GlJmFnA4IqJY4o2QEcKXgshJTjfNg"
    ...
    public function updateStockWithWebhook(Request $request) {
        if ($request->isMethod('post')) { // Check if the incoming HTTP Request Method/Verb is 'POST'
            ...
            // Check if the user sends the 'Authorization' HTTP Request Header or not with their HTTP Request (Check if the 'Authorization' Header is provided)
            if (empty($header)) { // if the user doesn't provide/send the 'Authorization' HT    TP Header with their HTTP Request
                $message = '\'Authorization\' HTTP Request Header value (i.e. Bearer Access Token) is missing!';
                return response()->json([ // JSON Responses: https://laravel.com/docs/9.x/responses#json-responses
                    'status'  => false,
                    'message' => $message
                ...
                // Check if the 'Authorization' HTTP Request Header is correct
                if ($header == 'Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJzdWIiOiIxMjM0NTY3ODkwIiwibmFtZSI6IkFtaXQgR3VwdGEiLCJpYXQiOjE1MTYyMzkwMjJ9.cNrgi6Sso9wvs4GlJmFnA4IqJY4o2QEcKXgshJTjfNg') { // if the user sends the correct HTTP Request Header value (i.e. correct Bearer Access Token) with their HTTP Request    // JWT token generated by (we wrote in "Mohamed Ashraf" this token in the website): https://jwt.io/    // User must send an "Authorization" HTTP Header with all their HTTP Requests with this value (Bearer Token (JWT)): "Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVC        J9.eyJzdWIiOiIxMjM0NTY3ODkwIiwibmFtZSI6IkFtaXQgR3VwdGEiLCJpYXQiOjE1MTYyMzkwMjJ9.cNrgi6Sso9wvs4GlJmFnA4IqJY4o2QEcKXgshJTjfNg"
                    $data = $request->all();
                    ...
                    return response()->json([ // JSON Responses: https://laravel.com/docs/9.x/responses#json-responses
                        'status'  => false,
                        'message' => 'Your submitted \'Authoriz            ation\' HTTP Request Header value (i.e. the Bearer Access Token) is incorrect!'
                    ], 422); // 422 HTTP Status Code: Unprocessable Content
                }
